"""
Torob Crawler - Crawler for Torob.com (product comparison)
"""
...
class TorobCrawler(BaseCrawler):
    """
    Crawler for Torob.com
    Product search and price comparison
    """
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """Initialize Torob crawler"""
        super().__init__(config)
        self.site_name = 'torob'
        self.base_url = self.config.get('base_url', 'https://torob.com')
        self._use_selenium = False  # Torob can work without JS
    
    def build_search_url(self, filters: Dict[str, Any]) -> str:
        """Build Torob search URL"""
    ...
    def extract_items(self, html_content: str) -> List[Dict[str, Any]]:
        """Extract products from Torob HTML"""
        items = []
